(function () {
  ...
  function initNavbar() {
    ...
    try {
      ...
      if (brand) {
        ...
        if (isFeature && hrefB.indexOf("../") !== 0) {
          brand.setAttribute("href", hrefB.replace(/^\./, ".."));
        }
      }

      // Fix logo image path for feature pages
      var logo = document.querySelector(".site-navbar .brand-logo");
      if (logo && isFeature) {
        var logoSrc = logo.getAttribute("src") || "";
        if (logoSrc.indexOf("../") !== 0) {
          logo.setAttribute("src", logoSrc.replace(/^\./, ".."));
        }
      }
  ...
  function ensureCookieBanner() {
    ...
    div.innerHTML =
      '<span>We use cookies for ads and measurement. See our <a href="' +
      (isFeature ? "../" : "./") +
  ...
  function loadFundingChoicesIfConfigured() {
    function load(id) {
      ...
      s.src =
        "https://fundingchoicesmessages.google.com/i/" + encodeURIComponent(id);
      document.head.appendChild(s);
    }
  ...
  if (!headerMount) {
    ...
    headerMount.setAttribute("data-include", "navbar");
  ...
  if (!footerMount) {
    ...
    footerMount.setAttribute("data-include", "footer");
  ...
  function setupStickyActions() {
    ...
    var sourceButtons = Array.prototype.slice
      .call(
        document.querySelectorAll("main button.primary, .card button.primary")
      )
      .filter(function (b) {
    ...
    sourceButtons.forEach(function (src) {
      ...
      var sync = function () {
        ...
        clone.disabled = !!src.disabled;
        clone.style.display =
          window.getComputedStyle(src).display === "none" ? "none" : "";
      };
      sync();
    ...
    function updateBar() {
      ...
      var anyVisible = sourceButtons.some(function (b) {
        return b.offsetParent && isMostlyVisible(b);
      });
      container.classList.toggle(
        "show",
        !anyVisible &&
          sourceButtons.some(function (b) {
            return b.offsetParent;
          })
      );
    }
    ["scroll", "resize", "orientationchange"].forEach(function (evt) {
      window.addEventListener(evt, updateBar, { passive: true });
    });
    updateBar();
  }
